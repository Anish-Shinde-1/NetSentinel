...
def add_rule():
    ...
    if application:
        if "chrome.exe" in application.lower():
            chrome_paths = [
                "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe",
                "C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe"
            ]
            for path in chrome_paths:
                command_in = f'netsh advfirewall firewall add rule name="Block Chrome" dir=in action=block program="{path}"'
                command_out = f'netsh advfirewall firewall add rule name="Block Chrome" dir=out action=block program="{path}"'
                os.system(command_in)
                os.system(command_out)
            os.system('netsh advfirewall firewall add rule name="Block QUIC" dir=out action=block protocol=UDP remoteport=443')
            cursor.execute("INSERT INTO blocked_ips (application) VALUES (?)", ("Chrome",))
            messagebox.showinfo("Success", "Blocked Chrome and QUIC protocol")
        else:
